...
The pane divides its rows itself rather than leave them to flex, and the replies are paid before the text grows. A long tweet with a quoted card under it used to take every row it wanted first, which left the reply strip a single row and drew the card as an empty border: 12 rows of text plus a 5-row quote card fills a 34-row pane on its own. The text scrolls under `Ctrl+S` and a reply card does not, so the text now keeps only what is left once the replies hold a whole card, down to a floor of three rows. An article is the exception, since it runs to thousands of characters and holds its rows. When the pane is short enough that even that leaves the strip under one card, the replies fall back to one line each, `▸ @handle  text`, with the marker on the one the arrows are on.
