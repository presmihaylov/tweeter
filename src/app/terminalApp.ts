...
import { renderApp, renderOnboarding } from './renderText.ts'
...
export const runTerminalApp = async (opts: TerminalAppOptions): Promise<void> => {
  ...
  if (!opts.profile) {
    rootText.content = renderOnboarding()
  ...
  const client = new TwitterClient({ authToken: opts.profile.authToken, ct0: opts.profile.ct0 })
  ...
  renderer.start()
  state = { ...state, status: `validating ${opts.profileName ?? opts.config.defaultProfile}` }
  rerender()
  session.auth = await client.checkAuth()
  state = { ...state, status: session.auth.ok ? `auth ok @${session.auth.username}` : session.auth.error }
  rerender()
  if (session.auth.ok) {
    await loadFeed(opts.config.ui?.defaultFeed === 'forYou' ? 'forYou' : 'following')
  }
}
